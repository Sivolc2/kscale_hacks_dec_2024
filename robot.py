...
class Robot:
    ...
    def set_initial_positions(self) -> None:
        ...
        Uses pre-calibrated reset positions from param.yaml that have been tested to provide
        a stable standing position for the robot.
        """
        logger.info("Setting initial standing positions...")
        
        # Get initial positions from params
        initial_positions = self.params['robot'].get('initial_positions', {})
        if not initial_positions:
            logger.warning("No initial positions found in params, using zeros")
            initial_positions = {joint.name: 0.0 for joint in self.joints}
